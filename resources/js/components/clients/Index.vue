                                    ...
                                    <td>
                                        <h2 class="table-avatar">
                                            <a href="javascript:void(0);"
                                               @click="$router.push({name: 'client',params: { id: client.id }})"
                                            >{{client.name}} <span>{{client.id}}</span></a>
                                        </h2>
                                    </td>
                                    <td>{{client.phone}}</td>
                                    <td><span class="badge badge-pill bg-success-light ">{{client.total_debits}}</span>
                                    </td>
                                    <td><span class="badge badge-pill bg-danger">{{client.total_credits}}</span>
                                    </td>
                                    <td>{{formatDate(client.created_at)}}</td>
                                    <td class="text-right">
                                        <a href="javascript:void(0);" class="btn btn-sm btn-white text-success me-2"
                                           @click="openInvoiceModal(client.id)">
                                            <i class="fas fa-file-invoice me-1"></i>Add Invoice
                                        </a>
                                        <a href="javascript:void(0);"
                                           class="btn btn-sm btn-white text-success me-2"
                                           @click="openPaymentModal(client.id,'debit')"
                                        >
                                            <i class="far fa-trash-alt me-1"></i>Add Debit
                                        </a>
                                        <a href="javascript:void(0);" class="btn btn-sm btn-white text-danger me-2"
                                           @click="openPaymentModal(client.id,'credit')"
                                        >
                ...
                <div class="modal-content rounded-0">
                    <div class="modal-body p-4 px-5">
                        ...
                        <div class="main-content text-center">
                            ...
                            <h4 class="card-title">{{form.id?'Update':'Create'}} Client</h4>
                            <div class="row">
                                <div class="col-md-6">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" v-model="invoiceForm.description">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>Rupees</label>
                                        <input type="number" class="form-control" v-model="invoiceForm.rupees">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
  ...
  export default {
    ...
    methods: {
      ...
      storeInvoice (print = false) {
        axios.post(`/api/store/client/invoice/${this.clientId}`, {
          ...
          if (print) {
            window.open(`/api/download/client/invoice/${res.data.invoice_id}`, '_blank');
          }
          this.reset();
        }).catch(error => {
          Swal.fire(
      ...
      createPayment () {
        axios.post('/api/store/client/payment', this.paymentForm).then(res => {
          Swal.fire(
            'Success',
            'Request has been executed.',
            'success',
      ...
      openPaymentModal (clientId, type) {
        this.paymentForm.client_id = clientId;
      ...
      addToCart () {
        if (!this.invoiceForm.date || !this.invoiceForm.rate || !this.invoiceForm.weight || !this.invoiceForm.rupees
          || !this.invoiceForm.description || !this.invoiceForm.amount) {
          Swal.fire(
            'Error',
          ...
          this.cart.push({
            ...
            rupees: this.invoiceForm.rupees,
      ...
      createUpdate () {
        this.$store.dispatch('clients/createUpdate', this.form);
        setTimeout(() => {
      ...
      remove (id) {
        return this.$store.dispatch('clients/delete', { id: id });
      },
      reset () {
        this.clientId = '';
        this.form.id = '';
    ...
    computed: {
      ...
      getCartSum () {
        let total = 0;
        this.cart?.forEach(c => total += +c.amount+ +c.rupees);
        return total;
      },
    },
  };
